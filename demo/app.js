...
function context(care=false){
  return `<div class="demo-context"><span class="eyebrow">${care?'CAREBRIDGE APP':'SOLACE APP'}</span>
    ...
    <div class="context-actions"><button class="primary" data-switch="${care?'home':'care'}">Switch to ${care?'Solace':'CareBridge'}</button><button class="secondary" data-reset>Reset demo</button><a class="secondary source-button" href="https://github.com/Sherron-T/Solace#readme" target="_blank" rel="noopener noreferrer">View project README ${icon('fa-arrow-up-right-from-square')}</a></div>
